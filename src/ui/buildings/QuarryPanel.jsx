...
export default function QuarryPanel({ data, onClose, destroyButton }) {
  const workers = data.workers || [];
  const canAssignVillager = workers.length < 2 && GameModel.gridData && hasAnyVillager();
  const canAssignMiner = workers.length < 2 && GameModel.professions.miner > 0;
  ...
  const assign = (type) => {
    Quarry.assignWorker(window.__phaserScene, data.rootX, data.rootY, type);
  };
  const unassign = () => {
    Quarry.unassignLastWorker(window.__phaserScene, data.rootX, data.rootY);
  };
  const pickTile = () => {
  ...
  const startAssignWarehouse = () => {
    ...
    window.__pickAssign = { x: data.rootX, y: data.rootY, type: "quarry" };
  };
  const deliver = () => {
    Quarry.deliverIfReady(window.__phaserScene, data.rootX, data.rootY);
  };
  ...
  return (
    <div style={panelStyle}>
      ...
      <div>Assigned mountain tile: {data.targetTile ? `${data.targetTile.x},${data.targetTile.y}` : "-"}</div>
      <div style={{ marginTop: 8 }}>
        <button style={btnStyle} disabled={!canAssignVillager} onClick={() => assign("villager")}>
          Assign villager (+15%)
        </button>
        <button style={btnStyle} disabled={!canAssignMiner} onClick={() => assign("miner")}>
          Assign miner (+50%)
        </button>
      ...
      <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <button
          ...
          onClick={pickTile}
          title={canPickTile ? "Click to choose a nearby mountain tile" : "Assign requires at least 1 worker; then choose a nearby mountain tile"}
        >
          Assign stone tile
        </button>
